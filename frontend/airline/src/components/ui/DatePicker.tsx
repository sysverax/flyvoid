...
export function DatePicker({ value, onChange, placeholder, align = "left", className }: DatePickerProps) {
  ...
  return (
    <div className={cn("relative", className ? "w-full block" : "inline-block w-[160px]")} ref={containerRef}>
      {/* Trigger input */}
      <div
        onClick={handleToggleOpen}
        className={cn(
          "relative cursor-pointer select-none flex items-center pr-12 text-left",
          className
            ? className.replace("block", "flex")
            : "h-11 w-full rounded-[8px] border border-[#D1D5DB] bg-[#F3F4F6] py-3 pl-4 text-gray-600 hover:bg-slate-100/80 transition-colors"
        )}
      >
